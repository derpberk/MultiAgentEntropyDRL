	...
	def train(self, episodes):
		...
		for episode in range(1, int(episodes) + 1):
			...
			while not done:
				...
				if self.safe_action:
					safe_mask = np.asarray([[self.env.get_action_mask(ind=j)] for j in range(self.env.number_of_agents)])

				# Process the agent step
				next_state, reward, done = self.step(action)

				if self.safe_action:
					safe_mask_ = np.asarray([[self.env.get_action_mask(ind=j)] for j in range(self.env.number_of_agents)])
